...
void print_hashmap(map<set<size_t>, int> *hashmap){
...
void print_cont_freq(){
    ...
    for (it = countmap.begin(); it != countmap.end(); it++){
        ...
        cout<<endl<<strong_rules_num;
        MyFile<<endl<<strong_rules_num;
    }
    MyFile.close();
}


void pruning(map<set<size_t>, int> initmap, map<set<size_t>, int> *hashmap, int min_s){
...
bool should_be_searched(int arr[], int k, map<set<size_t>, int> *hashmap)
{
    int count = pow(2, k);
    // set<size_t> subset;
...
void create_k_frequent(map<set<size_t>, int> *hashmap, vector<set<int> > transactions, int min_s){
    ...
    for (map<set<size_t>, int>::iterator first_itr = hashmap->begin(); first_itr!=hashmap->end(); first_itr++){        
        ...
        for (; sec_iter!=hashmap->end(); sec_iter++){
            ...
            // set<size_t> combined;
            // set_union(first_itr->first.begin(), first_itr->first.end(), sec_iter->first.begin(), sec_iter->first.end(), inserter(combined, combined.end()));
    ...
    hashmap->clear();     // clear the hashmap from k-1 itemsets, and add k-itemsets to that
    countmap.insert(pair<int, int> (k, tmpmap.size()));
    map<set<size_t>, int>::iterator iter= tmpmap.begin();;
	for(;iter != tmpmap.end(); iter++){
        hashmap->insert(pair<set<size_t>, int>(iter->first, iter->second));
        bigmap.insert(pair<set<size_t>, int>(iter->first, iter->second));
        frequents_vector.push_back(iter->first);         //append to all frequent itemsets vector
    }
...
*/
void generate_1_itemset_map(map<set<size_t>, int> *hashmap, vector<set<int> > transactions, int min_s){
...
int main(int argc, char **argv){
    ...
    create_k_frequent(hashmap, transactions, min_s);
